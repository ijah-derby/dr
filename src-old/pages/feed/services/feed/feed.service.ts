...
export class FeedService extends Extender {
  ...
  public getUserFeeds(uid: string, loader: any) {
    let feed: IFeed[] = [];
    // this.firestoreService.col$<IFeed>('feed', ref => ref.where('uid', '==', uid).orderBy('createdAt', 'desc').limit(10)).subscribe(feed => {
    //   console.log(feed);
    // });
    return this.firestoreService
      .col$<IFeed>(`feed`, (ref: any) =>
        ref.where("uid", "==", uid).orderBy("createdAt", "desc").limit(4)
      )
      .pipe(
        switchMap((_feed: IFeed[]) => {
          const reads = [];
  ...
  public getFeedNext(uid: string, lastInResponse: any) {
    return this.firestoreService
      .colWithIds$<IFeed>(`feed`, (ref: any) =>
        ref.orderBy("createdAt", "desc").limit(20).startAfter(lastInResponse)
      )
      .pipe(
        switchMap((_feed: IFeed[]) => {
          if (_feed.length !== 0) {
  ...
  public getFeedPrevious(uid: string, firstInResponse: any) {
    return this.firestoreService
      .colWithIds$<IFeed>(`feed`, (ref: any) =>
        ref.orderBy("createdAt", "desc").limit(20).endBefore(firstInResponse)
      )
      .pipe(
        switchMap((_feed: IFeed[]) => {
          const reads = [];
  ...
  public getPost(pid: string, uid: string) {
    ...
    return this.firestoreService.doc$(`feed/${pid}`).pipe(
      switchMap((_feed: IFeed) => {
        ...
        docs.push(
          this.firestoreService.colWithIds$(`feed/${pid}/comments`, (ref) =>
            ref.where("fid", "==", feed.id)
          )
        );
  ...
  public getReplies(fid: string, cid: string, uid: string) {
    return this.firestoreService
      .colWithIds$<IReply>(`feed/${fid}/comments/${cid}/replies`, (ref: any) =>
        ref.orderBy("createdAt", "asc")
      )
      .pipe(
        ...
        map((data: any) => {
          return (this.replies = this.replies.map(
            (item: IReply, index: number) => {
              return {
                ...item,
                user: data[index] || null,
              };
  ...
  public async upsertComment(comment: IComment) {
    ...
    await this.firestoreService.set(
      `feed/${commentObject.fid}/comments/${commentObject.id}`,
      commentObject
    );
  ...
  public async upsertReply(reply: IReply) {
    ...
    await this.firestoreService.set(
      `feed/${replyObject.fid}/comments/${replyObject.cid}/replies/${replyObject.id}`,
      replyObject
    );
    return this.firestoreService.update(
      `feed/${replyObject.fid}/comments/${replyObject.cid}`,
      {
  ...
  public async deleteReply(reply: IReply) {
    await this.firestoreService.delete(
      `feed/${reply.fid}/comments/${reply.cid}/replies/${reply.id}`
    );
    return this.firestoreService.update(
      `feed/${reply.fid}/comments/${reply.cid}`,
      {
  ...
  public getUpdateSpecificComment(comment: IComment): Observable<IComment> {
    return this.firestoreService.getSpecificDoc<IComment>(
      `feed/${comment.fid}/comments/${comment.id}`
    );
  }
}
