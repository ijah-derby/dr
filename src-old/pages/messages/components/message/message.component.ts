  ...
  */
  ...
  private async resetCounter() {
    if (this.message != null || this.message != undefined) {
      let snap = await this.firestore
        .doc(`messages/${this.message.id}`)
        .valueChanges();
      await snap.forEach(async (data) => {
        let obj = data as IMessage;
        if (
          this.active == 1 &&
          obj.last_sender != undefined &&
          obj.last_sender != this.currentUser.uid &&
          obj.seen != undefined &&
          obj.seen != true
        ) {
          await this.firestoreService.updateCounter(
  ...
  public call() {
    this.commonService.callUser(
      this.getSender(this.message).mobile || this.getSender(this.message).phone,
      this.callNumber
    );
  }
  ...
  public autoReply(text: any, images = null) {
    if (this.settingService.setting.autoReply === true) {
      setTimeout(() => {
        ...
        this.messageService
          .send({ ...this.message }, data)
          .then(() => this.loadingService.dismissLoader())
          .catch((err) => this.failPromise(err));
      }, 6000);
    }
  }
