...
// import { messages } from "platforms/android/app/build/intermediates/merged_assets/debug/mergeDebugAssets/out/www/assets/data/message";
...
export class MessagesComponent extends Extender implements OnInit {
  ...
  public getCounter(message: IMessage) {
    if (
      message.last_sender != this.currentUser.uid &&
      message.unread_count > 0
    ) {
      this.messageService.unreadCounter =
        this.messageService.unreadCounter + message.unread_count;
      return message.unread_count;
    } else {
      this.messageService.unreadCounter = 0;
      return 0;
    }
  }
    ...
    if (val && val.trim() !== "") {
      this.messages = this.allMessages.filter((item) => {
        return item.participants.find(
          (user) =>
            user.displayName.toLowerCase().indexOf(val.toLowerCase()) > -1
        );
      });
    } else {
