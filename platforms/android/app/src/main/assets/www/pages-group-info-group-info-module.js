...
var GroupInfoPage = /** @class */ (function () {
    ...
    GroupInfoPage.prototype.ionViewDidEnter = function () {
        ...
        this.dataProvider.getCurrentUser().subscribe(function (user) {
            _this.user = user;
            console.log('current user', _this.user.uid);
        });
    };
    // Delete subscription.
    // ionViewDidLeave() {
                    ...
                    handler: function () {
                        ...
                        _this.imageProvider.setGroupPhotoPromise(_this.group, _this.camera.PictureSourceType.CAMERA).then(function (group) {
                            ...
                            _this.firestore.update("groups/" + _this.groupId, {
                                ...
                                _this.loadingProvider.dismissLoader();
                                _this.firestore.set("groups/" + _this.groupId + "/messages/" + msgObj.id, msgObj);
                                _this.loadingProvider.showToast("Updated Successfully");
                            }).catch(function (error) {
                                _this.loadingProvider.dismissLoader();
                                _this.loadingProvider.showToast("Something went wrong");
            ...
            buttons: [
                ...
                },
                {
                    ...
                    handler: function (data) {
                        ...
                        var msgs = [];
                        _this.firestore.col$("groups/" + _this.group.id + "/messages").subscribe(function (resp) {
                            msgs = resp;
                        });
                        ...
                        _this.groupMembers.forEach(function (member, index) {
                            _this.firestore.delete('users/' + member + '/groups/' + group.id).then(function () {
    ...
    GroupInfoPage.prototype.askRemove = function (member) {
    ...
    GroupInfoPage.prototype.removeMember = function (member) {
        ...
        this.firestore.update("groups/" + this.groupId, {
            ...
            // Remove group from user's group list.
            _this.firestore.set("groups/" + _this.groupId + "/messages/" + msg.id, msg);
            _this.firestore.delete('users/' + member.uid + '/groups/' + _this.groupId).then(function () {
                // Pop this view because user already has left this group.
                _this.loadingProvider.dismissLoader();
                // this.group = null;
                // setTimeout(() => {
