...
import { IChat, IMessage, INotification } from '../../models/message';
...
export class MessagesComponent extends Extender implements OnInit {
  ...
  constructor(protected injector: Injector, private authService: AuthService, private messageService: MessagesService,
    private angularFire: AngularFirestore, private peopleService: PeopleService) {
    super(injector);
  }
  ...
  public getCounter(message: IMessage) {
    if((message.last_sender != this.currentUser.uid) && message.unread_count > 0) {
