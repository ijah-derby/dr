...
export class MessagesService extends Extender {
  ...
  /** send a message */
  public async send(message: IMessage, data: IChat) {
    delete message.participants;
    this.currentUser = await this.authService.getUser();
    let counter: number = 0;
    let snap = await this.angularFireStore.doc(`messages/${message.id}`).get();
    await snap.forEach( async data => {
      let obj = data.data();
      if (obj.unread_count != undefined) {
         counter = await obj.unread_count;
    ...
    return this.firestoreService.update(`messages/${message.id}`, {
      ...
      last_sender: this.currentUser.uid,
