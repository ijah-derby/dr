...
export class DashboardComponent extends Extender implements OnInit {
  ...
  async ngOnInit() {
    ...
    this.notificationService.getNotifications(this.currentUser.uid).subscribe((data) => {
      this.notificationCounter = this.notificationService.getUnreadCount([...data], this.currentUser.uid);
      this.loading = false;
    }, err => {
      this.loading = false;
    });
