import { Component, EventEmitter, Injector, Input, OnInit, Output, ViewChild, ElementRef } from '@angular/core';
...
export class MinimalComponent extends Extender implements OnInit {
  ...
  constructor(protected injector: Injector, private authService: AuthService, private modalController: ModalController, 
  ...
  public clicked(name: string, index: number, feed: IFeed): void {
    ...
    if ( name === 'like') {
      ...
      if (this.feed.userLiked) {
        ...
        this.feed.likes = this.feed.likes.filter((like) => like !== this.me.uid);
      }
    }
